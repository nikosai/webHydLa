...
function modifyNameLabel(name: string | null | undefined) {
  var text = "";
  ...
  ctx.fillText(text, 0, (<any>canvas).height - 50);
}
...
var plot_animate: any = [];

function replot(line: any) {
  remove_plot(line);
...
var PlotStartTime: number | undefined;
var array = -1;

function add_plot(line: any) {
  var axes;
  if (line.settings.x == "" ||
    line.settings.y == "" ||
    line.settings.z == "") {
  ...
  try {
    while (true) {
      ...
      if (phase.children.length == 0) { // on leaves
        ...
        var three_line: any = new THREE.Mesh(
          cylindersGeometry,
...
function phase_to_line_vectors(phase: Phase, parameter_condition_list: Env, axis: any, maxDeltaT: number) {
  ...
  if (phase.type == "PP") {
    ...
    var newPos: any = new THREE.Vector3(axis.x.getValue(env), axis.y.getValue(env), axis.z.getValue(env));
    newPos.isPP = true;
    line.push(newPos);
  } else {
...
function expandFrustum(orig: any, epsilon: any) {
...
function expandTwoPlanesOfFrustum(plane1: any, plane2: any) {
...
function getRangeOfFrustum(camera: any) {
  var xmin, ymin, zmin;
  ...
  var frustum: any = new THREE.Frustum();
