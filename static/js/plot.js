...
function translate_phase(phase) {
    ...
    for (var key in phase.variable_map) {
        ...
        phase.variable_map[key] = parseValue(phase.variable_map[key].unique_value);
    }
...
function updateAxisScaleLabel(xrange, yrange, zrange) {
    ...
    if (!plot_settings.scaleLabelVisible)
        return;
    ctx.font = "20px 'Arial'";
...
function replot_all() {
    ...
    for (var i in plot_lines) {
        plot_lines[i].color_angle = parseInt(i) / Object.keys(plot_lines).length * 360;
        replot(plot_lines[i]);
    }
    time = 0;
}
...
function add_plot(line) {
    ...
    if (line.plot_ready == undefined)
        requestAnimationFrame(function () { plot_ready(line); });
}
    ...
    try {
        while (true) {
            ...
            if (phase.children.length == 0) {
                ...
                for (var i = 0; i + 1 < current_line_vec.length; i++) {
                    if ('isPP' in current_line_vec[i + 1]) {
                        ...
                        for (var j = 1; j + 1 < numOfDots; j += 2) {
                            addCylinder(posBegin.clone().add(directionVec.clone().multiplyScalar(j * dottedLength)), posBegin.clone().add(directionVec.clone().multiplyScalar((j + 1) * dottedLength)));
                        }
                    }
                ...
                var three_line = new THREE.Mesh(cylindersGeometry, new THREE.MeshBasicMaterial({ color: color[current_param_idx] }));
...
function divideParameter(parameter_map) {
    ...
    for (var parameter_name in parameter_map) {
        ...
        if (setting.fixed) {
            ...
            for (var i = 0; i < now_parameter_condition_list.length; i++) {
                for (var j = 0; j < div; j++) {
                    ...
                    var tmp_obj = $.extend(true, {}, now_parameter_condition_list[i]); // deep copy
...
function phase_to_line_vectors(phase, parameter_condition_list, axis, maxDeltaT) {
    var line = [];
    var t;
    if (phase.simulation_state != "SIMULATED" && phase.simulation_state != "TIME_LIMIT" && phase.simulation_state != "STEP_LIMIT")
        return line;
    var env = {};
    $.extend(env, parameter_condition_list, phase.variable_map);
    if (phase.type == "PP") {
        env.t = phase.time.time_point;
        var newPos = new THREE.Vector3(axis.x.getValue(env), axis.y.getValue(env), axis.z.getValue(env));
        newPos.isPP = true;
        line.push(newPos);
    }
    else {
...
function check_parameter_condition(parameter_maps, parameter_condition_list) {
    ...
    for (var i = 0; i < parameter_maps.length; i++) {
        ...
        for (var key in parameter_maps[i]) {
            if (parameter_condition_list[key] === undefined)
                continue;
...
function getRangeOfFrustum(camera) {
    ...
    frustum = expandFrustum(frustum, undefined);
    ...
    var visible_x = Math.abs(d.y) + Math.abs(d.z) > epsilon, visible_y = Math.abs(d.z) + Math.abs(d.x) > epsilon, visible_z = Math.abs(d.x) + Math.abs(d.y) > epsilon;
    for (i = 0; i < intercepts.length; i++) {
        if (visible_x && !isNaN(intercepts[i].x))
            xmin = Math.min(xmin, intercepts[i].x);
        if (visible_y && !isNaN(intercepts[i].y))
            ymin = Math.min(ymin, intercepts[i].y);
        if (visible_z && !isNaN(intercepts[i].z))
            zmin = Math.min(zmin, intercepts[i].z);
        if (visible_x && !isNaN(intercepts[i].x))
            xmax = Math.max(xmax, intercepts[i].x);
        if (visible_y && !isNaN(intercepts[i].y))
            ymax = Math.max(ymax, intercepts[i].y);
        if (visible_z && !isNaN(intercepts[i].z))
            zmax = Math.max(zmax, intercepts[i].z);
    }
...
function calculate_intercept(point_a, point_b, point_c, point_d, frustum) {
    ...
    if (cross_product.x == 0)
        ...
        ret.x = sum / cross_product.x;
    if (cross_product.y == 0)
        ret.y = 0;
    else
        ret.y = sum / cross_product.y;
    if (cross_product.z == 0)
        ret.z = 0;
    else
        ret.z = sum / cross_product.z;
    if (!frustum.containsPoint(new THREE.Vector3(ret.x, 0, 0)))
        ret.x = Number.NaN;
    if (!frustum.containsPoint(new THREE.Vector3(0, ret.y, 0)))
        ret.y = Number.NaN;
    if (!frustum.containsPoint(new THREE.Vector3(0, 0, ret.z)))
        ret.z = Number.NaN;
    return ret;
}
    ...
    if (time_prev != time) {
        ...
        for (var i = 0; i < graph_scene.children.length - 1; i++) {
            if ('isLine' in graph_scene.children[i]) {
                ...
                if (time == 0) {
                    graph_scene.children[i + 1].material.color.set(animation_line[arr].color);
                }
                if (time > animation_line[arr].length - 1) {
                    graph_scene.children[i + 1].material.color.set(198, 198, 198);
                    plot_animate[arr] = (graph_scene.children[i + 1]);
                    arr += 1;
                    continue;
                }
                graph_scene.children[i + 1].position.set(animation_line[arr][time].x, animation_line[arr][time].y, animation_line[arr][time].z);
                plot_animate[arr] = (graph_scene.children[i + 1]);
                arr += 1;
            }
        }
...
function guard() {
    var g_geometry = new THREE.PlaneGeometry(100, 100, 100, 100);
    var g_material = new THREE.MeshBasicMaterial({
        color: 0x0000ff,
        wireframe: true
    });
    var guard = new THREE.Mesh(g_geometry, g_material);
...
function range_make_all() {
    ...
    if (animation_line.length != 0) {
        for (var j = 0; j < animation_line.length - 1; j++) {
            var face_geometry = new THREE.Geometry();
            var time_r = 0;
            for (var i = 0; i < animation_line.maxlen; i++) {
            ...
            for (var k = 0; k < face_geometry.vertices.length - 2; k++) {
